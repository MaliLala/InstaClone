import type { Post } from "./posts.types";

/**
 * Square card that scales image on hover and reveals caption overlay if present.
 */
export function PostCard({ post }: { post: Post }) {
  return (
    <div className="relative overflow-hidden rounded-lg shadow aspect-square group">
      <img
        src={post.imageUrl}
        alt={post.caption ?? "post"}
        className="object-cover w-full h-full transition-transform duration-300 group-hover:scale-105"
      />
      {post.caption ? (
        <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 transition-opacity duration-300 group-hover:opacity-100">
          <p className="text-white text-sm font-semibold p-2 text-center">
            {post.caption}
          </p>
        </div>
      ) : null}
    </div>
  );
}
